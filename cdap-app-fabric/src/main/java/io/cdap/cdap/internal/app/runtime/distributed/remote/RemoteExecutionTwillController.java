...
class RemoteExecutionTwillController implements TwillController {
  ...
  RemoteExecutionTwillController(ProgramRunId programRunId,
                                 ...
                                 ExecutorService executor, RemoteExecutionService service) {
    ...
    completion.whenComplete((c, t) -> {
      try {
        // Always issue an extra terminate call to make sure the remote process is stopped.
        // If the process is already stopped, it is a no-op
        remoteProcessController.terminate();
      } catch (Exception e) {
        LOG.warn("Exception raised when terminating remote execution for program {}", programRunId, e);
      }
      if (remoteProcessController instanceof Closeable) {
        Closeables.closeQuietly((Closeable) remoteProcessController);
      }
    });
